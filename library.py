...
import pandas as pd
import logging
from pepkit.process_lib import process_csv, process_fasta
from pepkit.utils import filter
...
def combine_files(
    ...
    for root, dirs, files in os.walk(input_folder):
        for file in files:
            ...
            if ext.lower() == ".csv":
                ...
                logger.warning(
                    f"Unsupported file type {ext} for file {file_path}. Skipping."
                )
    ...
    try:
        ...
        logger.error(
            f"Error writing to combined output file {combined_output_file}: {e}"
        )
...
def main():
    ...
    combined_log_file = "data/library/log_combined.txt"
    filtered_log_file = "data/screen_in/log_filtered.txt"
